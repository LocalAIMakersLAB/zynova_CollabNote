...
def today_local_iso(utc_offset_hours: int) -> str:
    """
    지정된 UTC 오프셋에 맞는 현재 날짜를 ISO 형식의 문자열로 반환합니다.
    한국(KST)은 UTC+9 입니다.
    """
    # 1. 현재 UTC 시간 가져오기
    ...
    # 3. 'YYYY-MM-DD' 형식으로 반환
    return local_now.strftime('%Y-%m-%d')

def get_due_todos_for_date(user_id: str, target_date: str, tz_hours: int = 9) -> List[Dict[str, Any]]:
    """
    특정 사용자의 특정 날짜에 마감인 Todo 목록을 가져옵니다.
    날짜 비교 시 타임존을 고려합니다.
    """
    try:
        # 1. 한국 시간(KST) 타임존 객체 생성
        kst = timezone(timedelta(hours=tz_hours))

        # 2. 검색할 날짜의 시작(00:00)과 끝(다음날 00:00) 시간 정의
        start_of_day = datetime.fromisoformat(target_date).replace(hour=0, minute=0, second=0, microsecond=0, tzinfo=kst)
        end_of_day = start_of_day + timedelta(days=1)

        # 3. Supabase는 UTC 기준이므로, KST를 UTC로 변환하여 쿼리
        start_utc = start_of_day.astimezone(timezone.utc).isoformat()
        end_utc = end_of_day.astimezone(timezone.utc).isoformat()

        # 4. Supabase에서 데이터 조회
        res = supabase.table("todos") \
            .select("*") \
            .eq("owner", user_id) \
            .gte("due_at", start_utc) \
            .lt("due_at", end_utc) \
            .execute()

        return res.data or []
    except Exception as e:
        print(f"Error in get_due_todos_for_date: {e}")
        return []
